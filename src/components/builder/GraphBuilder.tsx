...
export function GraphBuilder({ value, onChange }: Props) {
  ...
  const yScale = (y: number) =>
    H - PAD - ((y - spec.yMin) / (spec.yMax - spec.yMin)) * (H - PAD * 2);
  const invX = (px: number) => spec.xMin + ((px - PAD) / (W - PAD * 2)) * (spec.xMax - spec.xMin);
  const invY = (py: number) =>
    spec.yMin + ((H - PAD - py) / (H - PAD * 2)) * (spec.yMax - spec.yMin);

  const dragRef = useRef<
    | {
        series: number;
        point: number;
        kind: "point" | "bar" | "barX" | "barW";
        startX?: number;
        startW?: number;
      }
  ...
  const onPointerDown = (
    ...
    si: number,
    pi: number,
    kind: "point" | "bar" | "barX" | "barW" = "point",
  ) => {
    e.preventDefault();
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (drag.kind === "pie") {
      ...
      slices[drag.boundary + 1] = {
        ...slices[drag.boundary + 1],
        value: +(next - clamped).toFixed(3),
      };
    ...
    if (drag.kind === "label") {
      ...
      if (drag.mode === "dot") labels[drag.index] = { ...lbl, x, y };
        ...
        labels[drag.index] = {
          ...lbl,
          offsetX: +(x - lbl.x).toFixed(2),
          offsetY: +(y - lbl.y).toFixed(2),
        };
      set({ labels });
      return;
    }
  ...
  const addPoint = () => {
    if (active.kind === "line" || active.kind === "scatter") {
      const points = [
        ...active.points,
        [(spec.xMin + spec.xMax) / 2, (spec.yMin + spec.yMax) / 2] as [number, number],
      ];
  ...
  const gridXs = useMemo(
    () => (spec.xTicks ?? []).map(xScale),
    [spec.xTicks, spec.xMin, spec.xMax],
  );
  const gridYs = useMemo(
    () => (spec.yTicks ?? []).map(yScale),
    [spec.yTicks, spec.yMin, spec.yMax],
  );
    ...
    <div className="space-y-3">
                ...
                return (
                  <g key={si}>
                    <path
                      ...
                      stroke={color}
                      strokeWidth={2}
                      strokeDasharray={s.dashed ? "6 5" : s.dotted ? "2 4" : undefined}
                    />
                    {s.points.map((p, pi) => (
                      <circle
                        key={pi}
                ...
                <g key={`lbl${li}`}>
                  ...
                  {lbl.showDot !== false && (
                    <circle
                      ...
                      strokeWidth={2}
                      className="cursor-grab"
                      onPointerDown={(e) => onPointerDownLabel(e, li, "dot")}
                    />
                  )}
                  <g
                    transform={`translate(${lx} ${ly})`}
                    className="cursor-grab"
                    onPointerDown={(e) => onPointerDownLabel(e, li, "text")}
                  >
                    <rect
                      x={-30}
                      y={-9}
                      ...
                      stroke="var(--border)"
                    />
                    <text x={0} y={3} textAnchor="middle" fontSize={10} fill="var(--foreground)">
                      {lbl.text || "label"}
                    </text>
                  </g>
                </g>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <TextField
          label="x label"
          value={spec.xLabel ?? ""}
          onChange={(v) => set({ xLabel: v || undefined })}
        />
        <TextField
          label="y label"
          value={spec.yLabel ?? ""}
          onChange={(v) => set({ yLabel: v || undefined })}
        />
        <TicksField
          label="xTicks"
          value={spec.xTicks ?? []}
          labels={spec.xTickLabels}
          onChange={(ticks, labels) => set({ xTicks: ticks, xTickLabels: labels })}
        />
        <TicksField
          label="yTicks"
          value={spec.yTicks ?? []}
          labels={spec.yTickLabels}
          onChange={(ticks, labels) => set({ yTicks: ticks, yTickLabels: labels })}
        />
        {spec.yLabel && (
          <NumField
            label="y lbl gap"
            value={spec.yLabelOffset ?? 32}
            onChange={(v) => set({ yLabelOffset: v })}
          />
        )}
        {spec.xLabel && (
          <NumField
            label="x lbl gap"
            value={spec.xLabelOffset ?? -10}
            onChange={(v) => set({ xLabelOffset: v })}
          />
        )}{" "}
      </div>
      ...
      <div className="flex items-center gap-2">
        <CustomCheckbox
          checked={!!spec.gridlines}
          onChange={(v) => set({ gridlines: v })}
          label="Gridlines"
        />
        <CustomCheckbox
      ...
      <div className="rounded-lg border border-border bg-muted/20 p-3">
        <div className="mb-2 flex items-center gap-2">
          ...
          <CustomSelect
            ...
            placeholder="Series"
            options={spec.series.map((s, i) => ({
              value: String(i),
              label: `${i + 1}: ${s.kind}${s.name ? ` (${s.name})` : ""}`,
            }))}
          ...
          <button
            type="button"
            onClick={addSeries}
            className="ml-auto inline-flex cursor-pointer items-center gap-1 rounded border border-border px-2 py-1 text-xs hover:bg-accent"
          >
            <LuPlus size={12} /> Series
          </button>
          <button
            type="button"
            onClick={() => removeSeries(seriesIdx)}
            className="inline-flex cursor-pointer items-center gap-1 rounded border border-destructive/50 px-2 py-1 text-xs text-destructive hover:bg-destructive/10"
          >
            <LuTrash2 size={12} />
          </button>
        </div>
        ...
        <div className="mb-2 flex flex-wrap items-center gap-2">
          <CustomRadio<GraphSeries["kind"]>
            ...
            onChange={(kind) => {
              ...
              if (kind === "bar")
                setSeries(seriesIdx, { kind: "bar", bars: [{ x: 1, y: 1 }], color: currentColor });
              else if (kind === "pie")
                setSeries(seriesIdx, {
                  kind: "pie",
                  slices: [
          ...
          <TextField
            ...
            value={active.name ?? ""}
            onChange={(v) =>
              setSeries(seriesIdx, { ...active, name: v || undefined } as GraphSeries)
            }
          />
          {active.kind !== "pie" && (
            <label className="inline-flex items-center gap-1 text-xs">
              ...
              <ThemeColorInput
                value={active.color}
                onChange={(v) =>
                  setSeries(seriesIdx, { ...active, color: v ?? "#3b82f6" } as GraphSeries)
                }
                fallback="#3b82f6"
                allowClear={false}
                size="md"
          ...
          {active.kind === "line" && (
            <>
              <CustomCheckbox
                checked={!!active.dashed}
                onChange={(v) => setSeries(seriesIdx, { ...active, dashed: v, dotted: false })}
                label="Dashed"
              />
              <CustomCheckbox
                checked={!!active.dotted}
                onChange={(v) => setSeries(seriesIdx, { ...active, dotted: v, dashed: false })}
                label="Dotted"
              />
              <CustomCheckbox
                checked={!!active.showPoints}
                onChange={(v) => setSeries(seriesIdx, { ...active, showPoints: v })}
                label="Show points"
              />
              <CustomCheckbox
                checked={!!active.smooth}
                onChange={(v) => setSeries(seriesIdx, { ...active, smooth: v })}
                label="Smooth (curved)"
              />
            </>
          )}
          {active.kind === "pie" && (
            <>
              <CustomCheckbox
                checked={!!active.donut}
                onChange={(v) => setSeries(seriesIdx, { ...active, donut: v })}
                label="Donut"
              />
              <CustomCheckbox
                checked={active.showLabels !== false}
                onChange={(v) => setSeries(seriesIdx, { ...active, showLabels: v })}
                label="Show labels"
              />
            </>
          )}
              ...
              <div key={i} className="flex items-center gap-1 text-xs">
                ...
                <button
                  ...
                  onClick={() =>
                    setSeries(seriesIdx, {
                      ...active,
                      slices: active.slices.filter((_, x) => x !== i),
                    })
                  }
            ...
            <button
              ...
              onClick={() =>
                setSeries(seriesIdx, {
                  ...
                  slices: [
                    ...active.slices,
                    { name: `Slice ${active.slices.length + 1}`, value: 1 },
                  ],
                })
              }
        ...
        {(active.kind === "line" || active.kind === "scatter") && (
          <div className="mb-2 flex flex-wrap items-center gap-2 rounded border border-border/50 bg-background p-2">
            <CustomSelect
              ...
              onChange={(v) =>
                setSeries(seriesIdx, { ...active, marker: v as MarkerShape } as GraphSeries)
              }
            />
            <label className="inline-flex items-center gap-1 text-xs">
              ...
              <input
                ...
                onChange={(e) =>
                  setSeries(seriesIdx, {
                    ...active,
                    markerSize: Number(e.target.value) || undefined,
                  } as GraphSeries)
                }
            ...
            {active.kind === "line" && (
              <>
                <CustomCheckbox
                  checked={!!active.lobf}
                  onChange={(v) =>
                    setSeries(seriesIdx, { ...active, lobf: v ? "linear" : undefined })
                  }
          ...
          <div className="space-y-1">
            ...
            <div className="grid grid-cols-2 gap-1 sm:grid-cols-4">
              {points.map((p, i) => (
                <div
                  key={i}
                  className="flex items-center gap-1 rounded border border-border bg-background p-1"
                >
        ...
        {active.kind === "bar" && (
          <div className="space-y-1">
            ...
            {active.bars.map((b, i) => (
              <div
                key={i}
                className="flex items-center gap-1 rounded border border-border bg-background p-1 text-xs"
              >
                <input
                  type="number"
                  value={b.x}
                ...
                <button
                  type="button"
                  onClick={() =>
                    setSeries(seriesIdx, { ...active, bars: active.bars.filter((_, x) => x !== i) })
                  }
                  className="cursor-pointer text-muted-foreground hover:text-destructive"
                >
                  <LuTrash2 size={11} />
            ...
            onClick={() =>
              set({
                labels: [
                  ...(spec.labels ?? []),
                  {
                    x: (spec.xMin + spec.xMax) / 2,
                    y: (spec.yMin + spec.yMax) / 2,
                    text: `L${(spec.labels?.length ?? 0) + 1}`,
                    arrow: true,
                    showDot: true,
                    offsetX: 1,
                    offsetY: 1,
        ...
        <div className="space-y-1">
          {(spec.labels ?? []).map((lbl, i) => {
            ...
            return (
              <div
                key={i}
                className="flex flex-wrap items-center gap-1 rounded border border-border bg-background p-1 text-xs"
              >
                <input
                  value={lbl.text}
                  placeholder="text"
                  onChange={(e) => updateLbl({ text: e.target.value })}
                ...
                <label className="inline-flex items-center gap-0.5">
                  ...
                  <input
                    type="number"
                    value={lbl.x}
                    onChange={(e) => updateLbl({ x: Number(e.target.value) })}
                    className="w-14 rounded border border-border bg-background px-1 py-0.5"
                  />
                </label>
                <label className="inline-flex items-center gap-0.5">
                  ...
                  <input
                    type="number"
                    value={lbl.y}
                    onChange={(e) => updateLbl({ y: Number(e.target.value) })}
                    className="w-14 rounded border border-border bg-background px-1 py-0.5"
                  />
                </label>
                <label className="inline-flex items-center gap-0.5">
                  ...
                  <input
                    type="number"
                    value={lbl.offsetX ?? 0}
                    onChange={(e) => updateLbl({ offsetX: Number(e.target.value) })}
                ...
                <label className="inline-flex items-center gap-0.5">
                  ...
                  <input
                    type="number"
                    value={lbl.offsetY ?? 0}
                    onChange={(e) => updateLbl({ offsetY: Number(e.target.value) })}
                    className="w-12 rounded border border-border bg-background px-1 py-0.5"
                  />
                </label>
                <CustomCheckbox
                  checked={!!lbl.arrow}
                  onChange={(v) => updateLbl({ arrow: v })}
                  label="arrow"
                />
                <CustomCheckbox
                  checked={lbl.showDot !== false}
                  onChange={(v) => updateLbl({ showDot: v })}
                  label="dot"
                />
                <ThemeColorInput
                  value={lbl.color}
                  onChange={(v) => updateLbl({ color: v })}
                  fallback="#111827"
                />
                <button
                  type="button"
                  onClick={() => set({ labels: (spec.labels ?? []).filter((_, x) => x !== i) })}
          ...
          {(spec.labels ?? []).length === 0 && (
            <div className="text-[11px] italic text-muted-foreground">
              No labels. Add one above; drag the dot to move the point, drag the pill to move the
              label offset.
            </div>
          )}
        </div>
...
function TicksField({
  ...
  value: number[];
  labels?: string[];
  onChange: (ticks: number[], labels: string[] | undefined) => void;
}) {
  // Use free-form text so commas + decimals can be typed freely; parse on blur.
  // Preserve the user's exact tokens ("2.0", "1.0") as display labels.
  const initial = labels && labels.length === value.length ? labels.join(", ") : value.join(", ");
    ...
    <label className="flex items-center gap-1 text-xs">
      ...
      <input
        ...
        onBlur={() => {
          ...
          const pairs = tokens.map((t) => ({ t, n: Number(t) })).filter((p) => !Number.isNaN(p.n));
          const nums = pairs.map((p) => p.n);
          const lbls = pairs.map((p) => p.t);
          const anyCustom = pairs.some((p) => p.t !== String(p.n));
          onChange(nums, anyCustom ? lbls : undefined);
          setText(lbls.join(", "));
        }}
        placeholder="0, 2.0, 4"
        className="w-full rounded border border-border bg-background px-1 py-1"
